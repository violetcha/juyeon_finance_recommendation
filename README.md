...
이번 프로젝트는 단순히 금융 상품을 보여주는 화면을 만드는 것보다, 사용자가 실제로 금융생활을 시작할 때 겪는 고민을 하나의 서비스 흐름으로 연결하는 데 중점을 두었습니다.

예적금 상품 비교, 맞춤 추천, 주거래은행 탐색, 지도 기반 지점 검색, 커뮤니티, 챗봇 기능을 하나의 서비스 안에 넣으면서 데이터 모델링과 API 설계의 중요성을 체감했습니다. 특히 외부 API를 직접 연동하면서 API 장애, 인증 오류, 데이터 중복 저장, fixture fallback 같은 시연 안정화 문제를 함께 고려해야 했습니다.

추천 알고리즘은 단순 최고금리 정렬만으로는 사용자에게 충분히 맞춤형 경험을 제공하기 어렵다는 점을 확인했고, 기간, 가입 방식, 우대조건, 사용자 프로필을 함께 반영하는 방식으로 개선했습니다.

결과적으로 **주연**은 금융 초보자가 예적금 상품을 비교하는 데서 끝나는 서비스가 아니라, 자신의 조건에 맞는 금융 선택을 하고 금융 개념을 이해하며 다른 사용자와 경험을 공유할 수 있는 금융생활 가이드 서비스로 확장되었습니다.
